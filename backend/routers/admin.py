...
from models.marketplace import AdminLotUpdate, AdminSummary, AdminWinner, BuyerProfile, BuyerVerificationUpdate
...
async def admin_buyers(status: str = "PENDING"):
    query = {"verification_status": status.upper()} if status != "all" else {}
...
async def update_buyer_verification(buyer_id: str, input: BuyerVerificationUpdate):
    ...
    document = await db.buyers.find_one_and_update(
        ...
        {"$set": {"verification_status": input.verification_status}},
